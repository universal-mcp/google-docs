...
@pytest.fixture
def app_instance():
    """Provides a GoogleDocsApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return GoogleDocsApp(integration=mock_integration)

def test_universal_mcp_google_docs_app_initialization(app_instance):
    """
    Test that the GoogleDocsApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "google-docs", "GoogleDocsApp instance has unexpected name."


def test_universal_mcp_google_docs_tool_docstrings_format(app_instance):
    """
    Test that each tool method in GoogleDocsApp has a well-formatted docstring,
...
def test_universal_mcp_google_docs_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in GoogleDocsApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
